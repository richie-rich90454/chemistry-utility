...
// Mock all calculator module imports
...
describe("eventListeners", () => {
    ...
    describe("calculator button wiring", () => {
        ...
        it("bond type button catches errors and displays in result", () => {
    ...
    describe("select change handlers", () => {
        ...
        it("calculation-type change calls getCalculationType when equation is not empty", () => {
            const input = document.getElementById("stoich-equation-input") as HTMLInputElement;
            input.value = "H2 + O2 -> H2O";
        ...
        it("calculation-type change does not call getCalculationType when equation is empty", () => {
            const input = document.getElementById("stoich-equation-input") as HTMLInputElement;
            input.value = "";
    ...
    describe("Enter key support", () => {
        ...
        it("Enter key on dilution input triggers calculateDilution", () => {
            simulateEnterKey("dilution-M1");
            expect(calculateDilution).toHaveBeenCalled();
        });

        it("Enter key on ideal gas input triggers calculateIdealGasLaw", () => {
            simulateEnterKey("ideal-P");
            expect(calculateIdealGasLaw).toHaveBeenCalled();
        });

        it("Enter key on bond predictor input triggers predictBondType", () => {
            simulateEnterKey("element1-input");
            expect(predictBondType).toHaveBeenCalledWith(mockElements);
        });

        it("non-Enter key does not trigger handler", () => {
            simulateNonEnterKey("dilution-M1");
            expect(calculateDilution).not.toHaveBeenCalled();
        });

        it("Enter key on electrolysis input triggers calculateElectrolysis", () => {
            simulateEnterKey("electrolysis-m");
            expect(calculateElectrolysis).toHaveBeenCalled();
        });

        it("Enter key on Nernst input triggers calculateNernst", () => {
            simulateEnterKey("E-standard");
            expect(calculateNernst).toHaveBeenCalled();
        });

        it("Enter key on combined gas input triggers calculateCombinedGasLaw", () => {
            simulateEnterKey("combined-P1");
            expect(calculateCombinedGasLaw).toHaveBeenCalled();
        });

        it("Enter key on Van der Waals input triggers calculateVanDerWaals", () => {
            simulateEnterKey("vdw-V");
            expect(calculateVanDerWaals).toHaveBeenCalled();
        });

        it("Enter key on cell potential input triggers calculateCellPotential", () => {
            simulateEnterKey("E1");
            expect(calculateCellPotential).toHaveBeenCalled();
        });

        it("Enter key on mass percent input triggers calculateMassPercent", () => {
            simulateEnterKey("mass-solute");
            expect(calculateMassPercent).toHaveBeenCalled();
        });

        it("Enter key on mixing input triggers calculateMixing", () => {
